...
class Game {
    ...
    ajustSize() {
        this.tileSize = this.canvas.width/(this.GRIDWIDTH+4);
        if (this.canvas.height/(this.GRIDHEIGHT+2) < this.tileSize) {this.tileSize = this.canvas.height/(this.GRIDHEIGHT+2);}
        this.gameCoords = {
            x: (this.canvas.width-this.tileSize*(this.GRIDWIDTH+4))/2,
            y: 0,
        };
    }
    ...
    onButton(clickX, clickY, buttonX, buttonY) {
        return this.gameCoords.x+this.tileSize*buttonX <= clickX && clickX < this.gameCoords.x+(buttonX+1.5)*this.tileSize && this.gameCoords.y+buttonY*this.tileSize <= clickY && clickY < this.gameCoords.y+(buttonY+1.5)*this.tileSize;
    }

    click(x, y) {
        if (this.onButton(x, y, 0, this.GRIDHEIGHT-0.25)) {this.moveLeft();}
        else if (this.onButton(x, y, 2, this.GRIDHEIGHT-0.25)) {this.moveRight();}
        else if (this.onButton(x, y, this.GRIDWIDTH+1.5, this.GRIDHEIGHT-0.25)) {this.turnRight();}
        else if (this.onButton(x, y, this.GRIDWIDTH-0.5, this.GRIDHEIGHT-0.25)) {this.turnLeft();}
        else if (this.onButton(x, y, this.GRIDWIDTH+1.5, this.GRIDHEIGHT-2.25)) {this.moveDown();}
    }
    ...
    displayButtons() {
        this.displayRect(0, this.GRIDHEIGHT-0.25, 1.5, 1.5, "rgba(0, 0, 0, 0)");
        this.displayImage(this.sprites.arrow, 0.75, this.GRIDHEIGHT+0.5, 1, 1, -Math.PI*0.5);
        this.displayRect(2, this.GRIDHEIGHT-0.25, 1.5, 1.5, "rgba(0, 0, 0, 0)");
        this.displayImage(this.sprites.arrow, 2.75, this.GRIDHEIGHT+0.5, 1, 1, Math.PI*0.5);
        this.displayRect(this.GRIDWIDTH+1.5, this.GRIDHEIGHT-0.25, 1.5, 1.5, "rgba(0, 0, 0, 0)");
        this.displayImage(this.sprites.turn, this.GRIDWIDTH+0.25, this.GRIDHEIGHT+0.5, 1, 1);
        this.displayRect(this.GRIDWIDTH-0.5, this.GRIDHEIGHT-0.25, 1.5, 1.5, "rgba(0, 0, 0, 0)");
        this.displayImage(this.sprites.turn, this.GRIDWIDTH+2.25, this.GRIDHEIGHT+0.5, 1, 1, 0, true);
        this.displayRect(this.GRIDWIDTH+1.5, this.GRIDHEIGHT-2.25, 1.5, 1.5, "rgba(0, 0, 0, 0)");
        this.displayImage(this.sprites.arrow, this.GRIDWIDTH+2.25, this.GRIDHEIGHT-1.5, 1, 1, Math.PI);
    }
    ...
    displayGameOver() {
        ...
        this.ctx.fillText("Vous avez obtenu un score de "+this.score, this.canvas.width/2, this.canvas.height/2-this.tileSize*(1+0.5*(this.score < this.requiredScore)));
        this.ctx.fillText(this.score >= this.requiredScore ? 'Vous avez validé le mini-jeu !' : 'Il vous faut un score d\'au moins '+this.requiredScore, this.canvas.width/2, this.canvas.height/2-this.tileSize*0.5*(1+(this.score < this.requiredScore)));
        if (this.score < this.requiredScore) {this.ctx.fillText('pour valider le mini-jeu', this.canvas.width/2, this.canvas.height/2-this.tileSize*0.5);}
        this.ctx.fillStyle = "#ff0000";
        this.ctx.font = this.tileSize.toString()+"px Sans-serif";
        this.ctx.fillText("GAME OVER", this.canvas.width/2, this.canvas.height/2-this.tileSize*(2+0.5*(this.score < this.requiredScore)));
    }
};
...
export default function TetrisContainer({navigation, handleValidate}) {
    ...
    const handlePlay = () => {
        setOver(false);
        if (game === null) {setGame(new Game(ref.current, handleEnd, requiredScore));}
        else {
            game.init();
            game.run();
        }
    ...
    const handleClick = e => {
        if (game) {
            ...
            game.click(locationX, locationY);
        }
    }
            ...
            </TouchableWithoutFeedback>
            {
                ...
                : (
                    <View style={{
                        ...
                        {over ? <TextButton expand={true} func={() => navigation.navigate('Map')}>Retour à la carte</TextButton> : null}
                        <TextButton expand={true} func={handlePlay}>{over ? 'Rejouer au mini-jeu' : 'Jouer au mini-jeu'}</TextButton>
                    </View>
                )
            }
        </View>
