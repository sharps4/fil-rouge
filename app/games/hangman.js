...
import DATA from '../data/hangman.json';

export default function Hangman({navigation, set, handleValidate}) {
    ...
    const handlePlay = () => {
        ...
        let choosen = keys[parseInt(Math.random()*keys.length)].toUpperCase();
        setWord(choosen);
        setCurrent('_'.repeat(choosen.length));
        setTested('');
        setLives(8);
        setPlaying(true);
    };
        ...
        <View style={{
        ...
        }}>
            {
                playing ? (
                        ...
                        <View style={{
                        ...
                        }}>
                            {
                                'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map((char, i) => 
                                    ...
                                    ? null
                                    : <TextButton key={i} func={() => handleTest(char)}>{char}</TextButton>
                                )
                            }
                        </View>
                    </>
                    ...
                    <>
                        <Paragraph color='#000' center={true}>{found ? 'Vous avez validé le pendu !' : 'Vous n\'avez pas validé le pendu'}</Paragraph>
                        <Paragraph color='#000' center={true}>Le mot était {word} ({DATA[set][word.toLowerCase()]})</Paragraph>
                        <TextButton expand={true} func={() => navigation.navigate('Map')}>Retour à la carte</TextButton>
                        <TextButton expand={true} func={handlePlay}>Rejouer au pendu</TextButton>
                    </>
                ) : <TextButton expand={true} func={handlePlay}>Jouer au pendu</TextButton>
            }
        </View>
    );
}